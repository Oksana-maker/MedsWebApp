...
namespace ApiTests
{
    public class Tests
    {
        ...
        {
            var json = JsonConvert.SerializeObject(new { login = "ok170519841@gmail.com", password = "1234", name = "Оксана" });
        ...
        public async Task Search()
        {
            client.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", access_token);
            int page = 1;
            var response = await client.GetAsync($"api/search/Амізончик/{page}");
            if (!response.IsSuccessStatusCode)
            {
                var errorString = await response.Content.ReadAsStringAsync();
                Assert.Fail(response.StatusCode.ToString() + errorString);
